	...
	class Module extends \Dns_Module implements ProviderInterface
	{
		...
		protected function zoneAxfr($domain): ?string
		{
			...
			try {
				...
				$soa = $this->get_records_external('', 'SOA', $domain, $this->get_hosting_nameservers($domain))[0] ?? null;
				if (!$soa) {
					return null;
				}
		...
		protected function atomicUpdate(string $zone, Record $old, Record $new): bool
		{
			...
			try {
				...
				$api->do('POST', 'dns/update_record', ['domain' => $zone, 'RECORDID' => $id] + $this->formatRecord($new));
			} catch (ClientException $e) {
